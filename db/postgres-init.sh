...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
  ...
  CREATE SCHEMA IF NOT EXISTS auth AUTHORIZATION $PG_USER;

  GRANT USAGE, CREATE ON SCHEMA core TO $PG_USER;
  GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA core TO $PG_USER;

  GRANT USAGE, CREATE ON SCHEMA auth TO $PG_USER;
    GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA auth TO $PG_USER;
